...
def main_loop():
    ...
    if detect_faces:
        processed_image, faces = detect_and_draw_faces(processed_image)
        if selected_face_filter:
            processed_image = apply_face_filter(processed_image, faces, selected_face_filter)
...
def apply_face_filter(image, faces, selected_filter):
